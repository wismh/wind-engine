...
std::optional<SplashDocument> build_splash_document(const SplashScreen& config) {
    if (!config.enabled) {
        return std::nullopt;
    }
    ...
    const std::string css = std::format(
            ...
            "  100% {{ opacity: 0; }}\n"
            "}}\n"
            ".{3} {{ animation-name: {0}; animation-duration: {4}s; width: 100%; height: 100%; }}\n",
    ...
    return SplashDocument{std::move(*document), std::move(*stylesheet)};
}
