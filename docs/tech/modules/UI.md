...
- `{binding path}` on `text` / `content` / `command` / `paint` / `source` / `items_source`, interned to a `BindingId` ([[include.engine.ui.binding_id.h]]) at parse time — not a runtime string lookup.
...
- [[src.ui.canvas.cpp]] — begin_frame, handle_pointer, apply_canvas_fit, canvas_layout_space, `spawn_canvas`. Pointer hit-test relayouts through `layout_painter_for` ([[src.ui.painter.h]] `UiLayoutPainters`) so hug text uses the same measurer as `paint_document`.
...
- [[src.ui.paint.cpp]] — cascade (incl. `@media`), interaction flags (topmost-hit-only via `hit_test()`), text position from justify/align/text-align, opacity keyframe sampling, `z-index` sibling sort before recursing into children, `IUiPainter::apply_transform` for non-identity rotation/scale, calls `IUiPainter`. After background/border, invokes `Element::paint` (`IPaint`) if bound.
...
[[tests.ui_xml_test.cpp]] · [[tests.ui_builder_test.cpp]] · [[tests.ui_paint_binding_test.cpp]] · [[tests.ui_css_test.cpp]] · [[tests.mvvm_test.cpp]] · [[tests.ui_painter_test.cpp]] · [[tests.assets_test.cpp]] (codegen) · [[tests.splash_test.cpp]]
