...
`handle_key` / `handle_mouse_button` / `handle_mouse_move` / `handle_mouse_wheel` emit `InputEvent` (by `ActionId`, if bound) / `MouseEvent` (always, for UI/pick; `Kind::Wheel` carries `wheel_y`) on the world ([[include.engine.core.input_system.h]]). Touch: `handle_touch` / `handle_touch_move` — the primary finger also synthesizes left-mouse Down/Move/Up for UI; a bound `ControlKind::Touch` emits `InputEvent`. `is_held(ActionId)` queries held state. Remap queries: `bound_action(Control|KeyCode|MouseButton)`, `controls_for(ActionId)`, `unbind(...)`.
