...
**Custom properties (`var-<name>` / `--name` / `var()`):** a generic escape hatch so an arbitrary CSS value (color, a length, anything `apply_declaration` parses) can carry MVVM data, without adding a dedicated `_binding` field to `Element` per property. `var-<name>="{binding path}"` on any XML element (`var-`, not `--`, because XML names can't start with `-`) resolves every frame in `bind_element` — same pass as `text`/`content`, so it tracks live VM changes exactly like they do — into `Element::custom_properties["<name>"]`. A stylesheet declaration's value of `var(--name)` (optionally `var(--name, fallback)`) is substituted in `compute_style` before the normal per-property parser runs: the element's own `custom_properties` wins (per-instance, like an inline override), then a cascaded `--name: value;` declaration elsewhere in the stylesheet (matched by the same class/id/specificity rules as everything else), then the `var()` fallback, else empty. Cascade-level `--name:` custom properties are resolved in single-pass declaration order (not full two-phase CSS semantics) — declare before use within a rule set. `@import`: **not v1**.
